...
import { InTrack } from "./Poster";

const Track: React.FC<{ track: InTrack; spotifyApi: SpotifyWebApi }> = ({
  track,
  spotifyApi,
}) => {
  ...
  const [hasLiked, setHasLiked] = useState(
    favorites.find((e: any) => e.id === track.id) ? true : false
  );
  ...
  return (
    <div className="flex items-center  w-full justify-between space-x-20 cursor-default py-2 px-4 rounded-lg group transition ease-out">
      <div className="flex items-center">
        ...
        <div>
          <h4
            onClick={() => handlerSong(track.id)}
            className="text-[#008c44] cursor-pointer text-sm hover:underline font-semibold truncate w-[450px]"
          >
            {track.title}
          </h4>
        </div>
